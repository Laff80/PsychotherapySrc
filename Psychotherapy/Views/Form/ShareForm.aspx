<%@ Page Language="C#" Inherits="System.Web.Mvc.ViewPage<IPagedList<Insourcia.Psychotherapy.Model.UserFormDataPermission>>" %>
...
<div id="middle_content_main"> 
        ...
        <!--选择条件结束-->
        <!--显示列表样式-->
        <form id="permissionForm" method="post" action="/Form/AddFormPermission">
         <%=Html.Hidden("hdPatientID",ViewData["PatientID"]) %>
         <%=Html.Hidden("hdFormDataID",ViewData["FormDataID"]) %>
        <div class="pagedData_box">
        <%Html.RenderPartial("UcFormPermissionList", Model); %>
        </div> 
        </form>
</div>
</body>
