<%@ Page Language="C#" Inherits="System.Web.Mvc.ViewPage<IPagedList<Insourcia.Psychotherapy.Model.UserPatientPermission>>" %>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $("#slUserPmAdd").combobox({ comboxSelected: function (event, data) {
                ...
                tr += '<td><input type="checkbox" name="chkbDownload" id="chkbDownload' + value + '" onclick="SelectedBox(this,"' + value + '")" /></td>';
                tr += '<td><input type="checkbox" name="chkbCheck" id="chkAll' + value + '" onclick="SelectedBox(this,"' + value + '")" /></td>';
                tr += '<td><input type="checkbox" title="Is Applied Form" name="chkApplyForm" id="chkApplyForm' + value + '" onclick="SelectedBox(this,"' + value + '")" /></td>';
                tr += '<td><input type="checkbox" title="Is Applied TreatmentPlan" name="chkApplyTreatmentPlan" id="chkApplyTreatmentPlan' + value + '" onclick="SelectedBox(this,"' + value + '")" /></td>';
                tr += '<td><input type="checkbox" title="Is Applied ScannedForm" name="chkScannedForm" id="chkScannedForm' + value + '" onclick="SelectedBox(this,"' + value + '")" /></td>';
                tr += '<td><input type="checkbox" title="Is Applied Chart" name="chkApplyChart" id="chkApplyChart' + value + '" onclick="SelectedBox(this,"' + value + '")" /></td>';
                tr += '</tr>';
...
<div id="middle_content_main"> 
 ...
 <%--       <input class="btn_adduser vl" type="button" value="&nbsp;&nbsp;Add&nbsp;&nbsp;" id="btUserPmAdd" />--%>
       ...
       <%-- <input class="btn_adduser vl" type="button" value="&nbsp;Save&nbsp;" id="btSaveShare" />
        ...
        <!--选择条件结束-->
        <!--显示列表样式-->
        <form id="permissionForm" method="post" action="/Client/AddPatientPermission">
         <%=Html.Hidden("hdPatientID",ViewData["PatientID"]) %>
        <div class="pagedData_box">
        <%Html.RenderPartial("UcPatientPermissionList", Model); %>
        </div> 
        </form>
</div>
</body>
